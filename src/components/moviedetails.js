...
export default function MovieDetails() {
    ...
    const copyMovieLink = () => {
        setPopUpShow(true);
        navigator.clipboard.writeText(API_BASE_URL+'/?i='+imdbID+'&apikey=aa660442');
        setTimeout(()=>{
            setPopUpShow(false);
        },2000)
    }
    ...
    useEffect(() => {
        const getData = async() => {
            ...
            console.log(imdbID)
            let response = await axios.get(API_BASE_URL+'/?i='+imdbID+'&apikey=aa660442');
    ...
    return (
        <div className='movieDetail'>
            <div className='shareBtn' onClick={copyMovieLink}>Copy Movie Link</div>
            {
                popUpShow && 
                <div className='popup'>Movie Link copied to clipboard</div>
            }
            <img src={poster} />
